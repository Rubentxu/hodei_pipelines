...
import dev.rubentxu.hodei.pipelines.infrastructure.script.PipelineContext
import dev.rubentxu.hodei.pipelines.infrastructure.script.PipelineScript
        ...
        try {
            ...
            result.reports.forEach { report ->
                if (report.severity >= ScriptDiagnostic.Severity.WARNING) {
                    val message = "[${report.severity}] ${report.message} (${report.sourcePath}:${report.location?.start?.line})"
                    diagnostics.add(message)
                    outputHandler(JobOutputChunk(message.toByteArray(), isError = report.severity >= ScriptDiagnostic.Severity.ERROR))
                }
            }
